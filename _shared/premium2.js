...
(function(){
  ...
  function finishIntro(){if(!intro)return;intro.classList.add('gone');setTimeout(()=>intro.remove(),400);}
  if(intro){
    const pct=intro.querySelector('.ipct'), bar=intro.querySelector('.ibar');
    let p=0;const t=setInterval(()=>{p+=Math.random()*28+18;if(p>=100){p=100;clearInterval(t);setTimeout(finishIntro,80);}
      if(pct)pct.textContent=Math.floor(p);if(bar)bar.style.width=p+'%';},90);
  }
